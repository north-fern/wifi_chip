...
def get_time():
    ...
    tot_sec = time_now_ms /1000
    hours = tot_sec / 360
    min_left = (tot_sec % 360) / 60
    sec_left = ((tot_sec % 360) % 60)
    time_now = str(hours) + ':' + str(min_left) + ':'+ str(sec_left)
...
def get_time():
    time_now_ms = utime.ticks_ms()
    tot_sec = time_now_ms /1000
    seconds_left_after_hours = (tot_sec - (tot_sec % 3600))
    hours = (tot_sec % 3600)
    sec_left_after_min = (seconds_left_after_hours - (seconds_left_after_hours % 60))
    min_left = (seconds_left_after_hours % 60)
    sec_left = sec_left_after_min
    time_now = str(hours) + ':' + str(min_left) + ':'+ str(sec_left)
